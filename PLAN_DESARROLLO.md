# Plan de Desarrollo — Tallerea MVP

Bitácora de fases ejecutadas. Documento canónico de fases: `Docs/AUDITORIA_Y_ARQUITECTURA.md`.

## Fases completadas

### Fase 0 — Base técnica ✅
**Commit:** `4878933`
- Env Zod (`src/lib/env.ts`)
- Migration script
- Vitest + 15 tests financieros

### Fase 1 — Refactor User + TallerService ✅
**Commits:** `c51b021`, `23f95dc`
- Modelo User rediseñado con `taller` embebido
- `TallerService` (CRUD + máquina de estados)
- Sesión NextAuth con `role`
- Migración aplicada en Atlas

### Fase 2 — Autenticación dual ✅
**Commits:** `62e6e2f`, `fb7c61a`
- Magic link alumnos (NextAuth EmailProvider via Resend)
- Registro tallerista con password
- Redirect post-login según `taller.estado`
- `magic/request` anti-enumeración (no auto-crea users)

### Fase 3 — Onboarding + Aprobación tallerista ✅
**Commits:** `088e34c`, `1735c89`
- `/tallerista/onboarding` (postulación)
- `/admin/talleristas` (aprobación/rechazo/suspensión)
- Bloqueo de re-postulación para suspendidos

### Fase 4 — Workshop refactor + Wizard ✅
**Commits:** `d40c3d8`, `dc224e4`
- Workshop apuntando a `ownerId` (User directo, sin Account)
- UI wizard nuevo taller
- Enum 15 categorías
- Validación `taller.estado === 'aprobado'` en POST

### Commit `4c7f34f` — Panel admin operacional ⚠️
**Rotulado "fase 5" pero corresponde realmente a:**
- **Fase 7 parcial** (panel tallerista/admin operacional): admin/finanzas, liquidaciones, strikes
- **Fase 10 parcial** (gestión de fricción): reembolsos como crédito

> Este commit se mantiene porque el código es válido, pero NO es la Fase 5 del documento canónico. La Fase 5 real (Checkout + Magic link alumno) se implementa en el siguiente commit.

### Fase 5 (real) — Checkout + Magic link alumno [CRÍTICO MVP] ✅
**Commit:** (este)

Cierra el flujo "alumno nace de transacción":
- `src/lib/guestUser.ts` — `findOrCreateGuestUser` para checkout invitado
- `src/lib/issueMagicLink.ts` — helper reutilizable (32 bytes hex + SHA256 + TTL 15min)
- `POST /api/{enrollments,subscriptions,payments/create}` aceptan `name+email` sin sesión
- `PaymentService.handleApprovedPayment` emite magic link tras pago si user es guest (`!password`)
- `sendEnrollmentConfirmation` con CTA "Activar mi cuenta" cuando hay `magicUrl`
- `/talleres/[slug]/inscribirse` con form name+email para guests (sin redirect a login)
- `/api/auth/magic/request` refactorizado para usar `issueMagicLink` (DRY)

## Fases pendientes

### Fase 6 — Modelo recurrente (Subscription + Booking) 🔄 parcial
- Modelos existen, falta UI completa de gestión por parte del alumno y tallerista
- **[BUSINESS LOGIC RISK]** `SubscriptionService.createWithPayment` pasa `subscription._id` como `enrollmentId` a MP, pero el webhook procesa solo via `EnrollmentService`. Suscripciones aprobadas hoy no completan flujo. Pendiente PR aparte.

### Fase 7 — Panel tallerista operacional ⚠️ parcial
- Admin de finanzas hecho (commit `4c7f34f`)
- Falta: dashboard tallerista con métricas de ocupación + agenda

### Fase 8 — Reviews
### Fase 9 — Cron de caducidad mensual (Subscription periodoFin → vencer Bookings → autoRenovar)
### Fase 10 — Gestión de fricción ⚠️ parcial
- Reembolsos = crédito (commit `4c7f34f`)
- Falta: política de no-show por workshop + flujo reagendamiento

### Fase 11 — SEO + landing pública pulida
### Fase 12 — QA + deploy producción

---

*Actualizar este archivo después de cada commit de fase.*
