...
export default async function WorkshopDetailPage({ params }: PageProps) {
  ...
  return (
    <>
            ...
            {owner && (
              <div>
                ...
                <div className="bg-purple-50 border border-purple-100 rounded-xl p-5">
                  ...
                  {/* Redes sociales */}
                  {(owner.taller?.redesSociales?.instagram || owner.taller?.redesSociales?.web || (owner.taller?.redesSociales?.whatsappEnabled && owner.taller?.redesSociales?.whatsapp)) && (
                    <div className="flex gap-3 mt-3">
                      ...
                      {owner.taller?.redesSociales?.whatsappEnabled && owner.taller?.redesSociales?.whatsapp && (() => {
                        const digits = owner.taller.redesSociales.whatsapp.replace(/\D/g, '')
                        const normalized = digits.length === 9 && digits.startsWith('9') ? `56${digits}` : digits
                        const msg = encodeURIComponent(`Hola, te escribo desde Tallerea por el taller "${workshop.titulo}".`)
                        return (
                          <a
                            href={`https://wa.me/${normalized}?text=${msg}`}
                            target="_blank"
                            rel="noopener noreferrer"
      ...
      {/* Botón flotante de WhatsApp del profesor */}
      {owner?.taller?.redesSociales?.whatsappEnabled && owner.taller.redesSociales.whatsapp && (() => {
        const digits = owner.taller.redesSociales.whatsapp.replace(/\D/g, '')
        const normalized = digits.length === 9 && digits.startsWith('9') ? `56${digits}` : digits
        if (normalized.length < 10) return null
        const msg = encodeURIComponent(`Hola ${owner.name}, te escribo desde Tallerea por el taller "${workshop.titulo}" (${`https://tallerea.cl/talleres/${workshop.slug}`}).`)
        return (
          <a
            href={`https://wa.me/${normalized}?text=${msg}`}
            target="_blank"
            rel="noopener noreferrer"
            aria-label="Escribir por WhatsApp al profesor"
            className="fixed bottom-5 right-5 z-50 flex items-center gap-2 bg-green-500 hover:bg-green-600 text-white px-4 py-3 rounded-full shadow-lg transition-colors"
          >
